...
const Quiz: React.FC<QuizProps> = ({ params }) => {
    ...
    const {
        ...
        queryKey: ['quiz-detail-info', { params: params?.quizId }],
        queryFn: () => examApi.getExamById(params?.quizId)
    });
    const { data: quizzesData } = useQuery<any>({
        queryKey: ['quiz-by-course', { params: params?.id }],
        queryFn: () => examApi.getQuizCourseById(params?.id)
    });
    const { data: videosCourse } = useQuery<any>({
        queryKey: ['my-course-videos', { params: params?.id }],
        queryFn: () => videoApi.getByCourseId(params?.id, 0, 100, 'id', 'ASC')
    });
    useEffect(() => {
    ...
    const {
        ...
        queryFn: () => examApi.getExamSubmissionByExamId(params?.quizId, page - 1, 5)
    });
    ...
    return (
        <VideoHeader courseId={params?.id}>
            <div className="w-[95%] 2xl:w-4/5 mx-auto">
                <div className="relative md:grid grid-cols-10 gap-2 mt-4 mb-16">
                        ...
                        {totalRow ? (
                            <ul className="p-3 sm:p-4 rounded-xl border-1 border-blue-500 shadow-xl w-full md:w-4/5 mt-4">
                                <li className="flex items-center justify-between">
                                    ...
                                    <Button
                                        as={Link}
                                        href={`/my-course/${params?.id}/quiz/${params.quizId}/practice`}
                                        size="sm"
                                        color="primary"
                    ...
                    <div className="hidden md:block h-full col-span-3">
                        <VideoList video={listVideo} courseId={params?.id} />
                    </div>
                    <Drawer
                    ...
                    >
                        <VideoList isOnDrawer={true} video={listVideo} courseId={params?.id} />
                    </Drawer>
                </div>
            </div>
        </VideoHeader>
